...
function getPhotos() {
    ...
    xhr.addEventListener("readystatechange", function () {
        if (this.readyState === this.DONE) {
            ...
            console.log(
                "the JSON after parsing, before sending to display",
                json
            );
...
function getUsers(data) {
    const userOutputDiv = document.getElementById("users");
    ...
    */
    for (let user of data.results) {
        ...
        let person = `<section><h3>${
            user.name.title + " " + user.name.first + " " + user.name.last
        }</h3><p>Username: ${user.login.username}</p><img alt="" src="${
            user.picture.large
        }" width="160" height="160"><p>Birthday: ${
            birthday.getMonth() + 1
        } - ${birthday.getDate()} - ${birthday.getFullYear()}</p><a href="mailto:${
            user.email
        }">${user.email}</a></section>`;
        text += person;
    }
    userOutputDiv.innerHTML = text;
}
